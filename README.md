...
>
> WeChat Publisher renders your Markdown notes into WeChat-compatible HTML with proper formatting — code highlighting, math formulas, Mermaid diagrams, local image upload, and one-click draft push to your Official Account. Free tier available (copy HTML + paste manually); paid tier pushes directly to your draft box via the WeChat API.
>
> 👇 中文文档见下方
...
- **免费使用**：格式预览 + 复制 HTML + 一键跳转公众号后台粘贴，全程不需要填写任何账号信息
- **付费直连**：配置公众号 AppID/AppSecret 后，直接从 Obsidian 推送到草稿箱，无需打开浏览器
...
**① 折叠控制栏**（始终显示）：顶部始终有两个按钮：**⊟ 收起工具栏**（点击后工具栏和发布资料区隐藏，预览区扩大；再次点击 **⊞ 展开工具栏** 恢复）和 **⟺ 开启同步**（开启后编辑器与预览双向联动滚动）。
...
折叠控制栏中有一个 **⟺ 开启同步** 按钮。开启后，编辑器与预览面板双向联动：滚动任意一侧，另一侧自动跟随。

**使用步骤：**

1. 点击 **⟺ 开启同步**，按钮变为"⟺ 同步中（请先滚动编辑器）"
2. 在左侧**编辑器中滚动一次**，按钮变为"⟺ 关闭同步"表示校准完成
3. 之后滚动任意一侧，另一侧自动跟随

同步优先按**标题段落对齐**，没有标题时按全文比例降级。再次点击按钮即可关闭同步。

### 复制HTML（免费）

点击顶部的**复制HTML（免费）**按钮，插件会将当前预览区的渲染结果复制为微信兼容的 HTML。

复制完成后，打开微信公众号后台编辑器，在正文区域直接粘贴即可。适合不需要直连 API、手动上传文章的场景。

### 去公众号粘贴（免费）

点击**去公众号粘贴（免费）**，浏览器会自动打开微信公众号后台 `mp.weixin.qq.com`，方便你在复制 HTML 后切换到后台粘贴。

### 账号配置（付费）

付费功能需要先完成**账号配置**和**激活码绑定**才能使用。

点击顶部操作栏中的**账号配置**按钮，打开配置弹窗。弹窗顶部以标签页（Tab）形式列出所有已添加的账号，点击标签可切换，右侧标有"已激活"/"未激活"状态。
...
| **账号授权** | 显示当前激活状态（✅ 已绑定 / 未绑定激活码），点击「刷新授权状态」可联网重新验证 |
