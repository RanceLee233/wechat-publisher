...
>
> v1.0.6 更新：修复正文中同一张图片出现多次时，部分图片未被替换、草稿里显示「图片未同步」的问题。
>
...
| 微信贴图 | 新建标准贴图笔记，预览并提交最多 20 张图片的微信图片消息 |
...
v1.1.0 起，插件可以直接把 Obsidian 笔记提交为微信公众号的贴图（图片消息）。工具栏仍只保留一个**发布草稿**按钮，插件会自动识别普通文章和贴图。

**使用步骤：**

1. 打开预览面板右上角的 **⋯ 更多**，点击**新建贴图**。
2. 插件会新建一个带标准字段的 Markdown 文件，把光标放在标题字段后。
3. 填写标题，将第一张图片放入封面区域，其余图片放入贴图区域；正文可以留空。
4. 在右侧确认图片顺序和数量后，点击**发布草稿**。
...
- 标题必填，直接写在 `## 标题：` 冒号后，最多 32 字。
- 封面必须且只能有 1 张，作为微信首图封面，并计入图片总数。
- 封面与贴图合计最多 20 张；超过限制时插件会中止发布并提示需要删除几张。
- 图片可以紧跟字段冒号，也可以放在下方；空行、文件名或路径里的空格不会影响识别。
- 图片始终按实际出现顺序发布，也兼容 `1. ![[图片]]` 这种自行编号写法。
- `## 正文：` 字段必须保留并放在最后，但正文内容可以为空；填写内容时会转换为微信支持的纯文本。
- 贴图使用专用预览，不使用文章主题和复制 HTML。相同图片会优先复用历史素材，同一笔记重复发布会自动更新原贴图草稿。
...
> v1.0.6 修复：正文中同一张图片出现多次时（例如首图在正文里重复使用），插件会只上传一次、但替换正文里的每一处显示，不再出现发布成功却有些图片显示「图片未同步，请在微信后台手动补传」的问题。
